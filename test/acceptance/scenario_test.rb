...
class ScenarioTest < AcceptanceTest
  ...
  test "scenario1 of release0_8" do
    ...
    # 1.4. エンジンを登録する
    fill_in "エンジン型式", with: "エンジンモデルコード2"
    ...
    # 1.5. エンジン一覧画面に戻る
    click_link "戻る"
    assert has_content? "エンジンモデルコード2"
    ...
    within "tbody > tr:nth-of-type(6)" do
      ...
      assert_equal "エンジンモデルコード2", nth_tag(:td, 2).text # エンジン型式
    ...
    select "エンジンモデルコード2", from: "search_engine_model_name"
    ...
    fill_in "search_serialno", with: "SN0001"
    click_button "検索"
    save_screenshot "scenario1-3_2_2.png"
    ...
    fill_in "search_serialno", with: "SN0001"
    click_button "検索"
    save_screenshot "scenario1-4_2_2.png"
    # 4.3. 整備完了画面を開く
    click_link "完了"
    ...
    all(:link_or_button, "引合登録")[1].click  # ボタンが二つあるので一個目をクリックする
    confirm
    ...
    within "tbody > tr:nth-of-type(2)" do
      assert_equal "エンジンモデルコード1 (00000001)", nth_tag(:td, 1).text # 返却エンジン
    ...
    all(:link_or_button, "受注登録")[1].click  # ボタンが二つあるので一個目をクリックする
    confirm
    ...
    select "エンジンモデルコード2", from: "engineorder_new_engine_attributes_engine_model_name"
    select "SN0001", from: "engineorder_new_engine_attributes_serialno"
    select "西宮戎リペア", from: "select_returning_place"

    save_screenshot "scenario1-7_3_2.png"
    all(:link_or_button, "引当登録")[1].click  # ボタンが二つあるので一個目をクリックする
    confirm
    ...
    within "table > tbody > tr:nth-of-type(2)" do
      assert_equal "エンジンモデルコード1 (00000001)", nth_tag(:td, 1).text # 返却エンジン
      assert_equal "エンジンモデルコード2 (SN0001)", nth_tag(:td, 2).text # 新規エンジン
    end
    ...
    fill_in "search_serialno", with: "SN0001"
    click_button "検索"
    save_screenshot "scenario1-8_2_2.png"
    ...
    comment = "通常便で送ります。"
    fill_in "出荷コメント", with: comment
    save_screenshot "scenario1-8_3_2.png"
    all(:link_or_button, "出荷登録")[1].click  # ボタンが二つあるので一個目をクリックする
    confirm
    ...
    select "2014", from: "engineorder_returning_date_1i"         # 返却日(年)
    select "2",    from: "engineorder_returning_date_2i"       # 返却日(月)
    select "15",   from: "engineorder_returning_date_3i"         # 返却日(日)
    comment = "天地無用でお願いします。"
    fill_in "返却コメント", with: comment                          # 返却コメント
    save_screenshot "scenario1-9_3_2.png"
    all(:link_or_button, "返却登録")[1].click  # ボタンが二つあるので一個目をクリックする
    confirm
    ...
    ###########################################################################
    # 10. 返却されたエンジン１を受領する [@SG]
    ###########################################################################
    # 10.1. サインインする
    sign_in "SG000001", "password"
    save_screenshot "scenario1-10_1.png"
    # 10.2. エンジン一覧画面を開く
    click_link "エンジン一覧"
    save_screenshot "scenario1-10_2.png"
    # 10.3. 返却エンジンをエンジン型式とエンジンNo.で検索する
    fill_in "search_engine_model_name", with: "エンジンモデルコード1"
    fill_in "search_serialno", with: "00000001"
    click_button "検索"
    save_screenshot "scenario1-10_3.png"

# 検索のバグが治ったら↓のコードをコメントから復帰させる
#    within "tbody > tr" do
#      assert_equal "受領前", nth_tag(:td, 1).text # ステータス
#      assert_equal "エンジンモデルコード1", nth_tag(:td, 2).text # エンジン型式
#      assert_equal "00000001", nth_tag(:td, 3).text # エンジンNo.
#      assert_equal "2014/02/15", nth_tag(:td, 4).text # 返却日
#      assert_equal "", nth_tag(:td, 5).text # 受領日はブランク
#      assert_equal "受領", nth_tag(:td, 7).text   # 作業登録は「受領」
#    end
